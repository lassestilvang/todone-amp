...
export const CalendarEventsList: React.FC<CalendarEventsListProps> = ({
  ...
  if (error) {
    return (
      <div className={cn('flex gap-3 rounded-lg border border-semantic-error bg-semantic-error-light p-4', className)}>
        <AlertCircle className="h-5 w-5 flex-shrink-0 text-semantic-error" />
        <div>
          <p className="text-sm font-medium text-semantic-error">Failed to load events</p>
